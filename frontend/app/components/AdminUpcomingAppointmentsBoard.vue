<template>
  <v-card class="upcoming-board rounded-xl border shadow-sm" flat>
      <v-card-title class="d-flex align-center pa-6 pb-4 flex-nowrap ga-2">
          ...
          <v-btn
              variant="text"
              class="month-label-btn text-h6 font-weight-bold text-grey-darken-4 flex-grow-1 text-center text-truncate"
              :aria-expanded="calendarDialogOpen"
      ...
      <v-card-text class="pa-6 pt-2">
          ...
          <div v-if="!loading && fetchError" class="text-body-2 text-error mb-4">{{ fetchError }}</div>

          <div
              v-if="!loading && !fetchError && orderedGroupKeys.length === 0"
              class="text-center py-10 text-grey-darken-1"
          >
          ...
          <div v-for="dateKey in orderedGroupKeys" :key="dateKey" class="mb-6">
              <div class="text-body-2 text-grey-darken-1 font-weight-medium mb-3">{{ sectionTitle(dateKey) }}</div>

              <div v-for="item in mappedGroups[dateKey]" :key="item.id" class="d-flex align-stretch ga-3 mb-3">
  ...
  <AdminAppointmentCalendarDialog
      v-model="calendarDialogOpen"
      :api-base="base"
      :title="monthLabel"
      :year="calendarAnchor.year"
      :month="calendarAnchor.month"
  />
</template>
...
<script setup>
  ...
  /** 0 = calendar week containing “today” in app timezone (see API `week_offset`). */
  ...
  const calendarAnchor = ref({ year: new Date().getFullYear(), month: new Date().getMonth() + 1 });
  ...
  function monthYearFromBoard() {
      if (days.value.length) {
          const [y, m] = days.value[0].date.split("-").map(Number);
  ...
  function openCalendarDialog() {
      calendarAnchor.value = monthYearFromBoard();
  ...
  /** All days in the loaded week that have appointments, chronological (API `Y-m-d` keys). */
  const orderedGroupKeys = computed(() => Object.keys(mappedGroups.value).sort());

  const listEmptyMessage = computed(() => "No appointments this week.");
  ...
  async function load() {
      ...
      try {
          ...
          groupedRaw.value =
              data?.grouped_appointments && typeof data.grouped_appointments === "object"
  ...
  /** Used when the dashboard refreshes data — always jump back to this week. */
  function reloadToCurrentWeek() {
      weekOffset.value = 0;
      return load();
  }
