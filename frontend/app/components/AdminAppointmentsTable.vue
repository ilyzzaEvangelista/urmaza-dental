  ...
  <v-card class="appointments-card rounded-xl border shadow-sm" flat>
      ...
      <v-card-title class="pa-6 d-flex flex-column flex-sm-row align-stretch align-sm-center flex-wrap ga-4">
          <div v-if="statusFilter" class="d-flex align-center ga-2 w-100 mb-1">
              <span class="text-subtitle-2 font-weight-bold text-grey-darken-4">Recent Appointments</span>
          </div>
          <v-text-field
              v-model="searchQuery"
              density="comfortable"
              ...
              style="min-width: min(100%, 220px); max-width: 500px"
          />
          <v-spacer></v-spacer>
          <v-btn
              color="primary-blue"
...
<script setup>
  const props = defineProps({
      ...
      /** Current page (1-based), from Laravel pagination */
      page: {
          type: Number,
          default: 1,
      },
      /** Last page number from Laravel `last_page` */
      totalPages: {
          type: Number,
          default: 1,
      },
      /** When set, parent should request only this status from the API (e.g. `pending`). */
      statusFilter: {
          type: String,
          default: undefined,
      },
